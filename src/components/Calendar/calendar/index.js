...
const viewPropTypes = typeof document !== 'undefined'
...
class Calendar extends Component {
  ...
  getAccessibilityLabel = (state, day) => {
    const { today } = XDate.locales[XDate.defaultLocale]
    const { formatAccessibilityLabel } = XDate.locales[XDate.defaultLocale]
  ...
  renderWeekNumber(weekNumber, diff) {
    return (
      <View style={{ flex: 1, alignItems: 'center' }} key={`week-container-${weekNumber}`}>
        {diff ? <Text style={{ color: COLORS.GRAY_MEDIUM, marginTop: 5, fontSize: 12 * rate }}>{weekNumber}</Text> : <Day
          key={`week-${weekNumber}`}
          theme={this.props.theme}
  ...
  renderWeek(days, id) {
    ...
    if (this.props.showWeekNumbers) {
      week.unshift(this.renderWeekNumber(days[days.length - 1].getWeek(), this.props.diff))
    }
  ...
  render() {
    ...
    const {
      firstDay, showSixWeeks, hideExtraDays, enableSwipeMonths,
    } = this.props
